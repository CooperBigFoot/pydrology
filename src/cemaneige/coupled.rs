...
use crate::gr6j::outputs::{Fluxes as GR6JFluxes, FluxesTimeseries as GR6JFluxesTimeseries};
...
/// Snow timeseries (aggregated).
#[derive(Debug)]
pub struct SnowTimeseries {
    pub pliq: Vec<f64>,
    pub psol: Vec<f64>,
    pub snow_pack: Vec<f64>,
    pub thermal_state: Vec<f64>,
    pub gratio: Vec<f64>,
    pub pot_melt: Vec<f64>,
    pub melt: Vec<f64>,
    pub pliq_and_melt: Vec<f64>,
    pub temp: Vec<f64>,
    pub precip: Vec<f64>,
}
...
impl SnowTimeseries {
    fn with_capacity(n: usize) -> Self {
        Self {
            pliq: Vec::with_capacity(n),
            psol: Vec::with_capacity(n),
            snow_pack: Vec::with_capacity(n),
            thermal_state: Vec::with_capacity(n),
            gratio: Vec::with_capacity(n),
            pot_melt: Vec::with_capacity(n),
            melt: Vec::with_capacity(n),
            pliq_and_melt: Vec::with_capacity(n),
            temp: Vec::with_capacity(n),
            precip: Vec::with_capacity(n),
        }
    }

    fn push(&mut self, f: &SnowFluxes) {
        self.pliq.push(f.pliq);
        self.psol.push(f.psol);
        self.snow_pack.push(f.snow_pack);
        self.thermal_state.push(f.thermal_state);
        self.gratio.push(f.gratio);
        self.pot_melt.push(f.pot_melt);
        self.melt.push(f.melt);
        self.pliq_and_melt.push(f.pliq_and_melt);
